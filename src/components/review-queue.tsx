  ...
  return (
    <div className="space-y-3">
      ...
      <Card className={cn("p-5", pending && "opacity-60")}>
        ...
        <h2 className="mt-2 text-[length:var(--text-lead)] font-semibold leading-tight tracking-tight">
          {topic.title}
        </h2>

        {topic.outcome ? (
          <p className="mt-2 max-w-[70ch] text-[length:var(--text-small)] leading-relaxed text-muted">
            {topic.outcome}
          </p>
        ) : null}
        ...
        <Link
          href={href}
          className="mt-3 inline-flex items-center gap-1 rounded text-[length:var(--text-small)] text-[var(--accent)] hover:underline focus-ring"
        >
          Open the topic — notes, resources, breakdown <ArrowRight size={13} />
        </Link>

        <div className="mt-5 border-t border-line pt-4">
          <p className="text-[length:var(--text-micro)] font-medium text-muted">
          ...
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            {SCALE.map((s) => (
              ...
              >
                <span className="text-[length:var(--text-small)] font-medium">{s.label}</span>
                <span className="text-[length:var(--text-micro)] text-subtle">back in {s.hint}</span>
              </button>
            ))}
          </div>
          <p className="mt-2 text-[length:var(--text-micro)] text-subtle">
      ...
      <div className="flex justify-between">
        ...
        {pending ? (
          ...
          <span className="inline-flex items-center gap-1.5 text-[length:var(--text-small)] text-[var(--good)]">
